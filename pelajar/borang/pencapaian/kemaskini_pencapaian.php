...
include '../../../config.php';
$result=mysqli_query($conn,"SELECT * FROM pencapaian LEFT JOIN pelajar ON pelajar.id_user=pencapaian.id_user WHERE id_kebolehan ='".$_GET['id_kebolehan']."'");
    ...
    <body>
      ...
      <div class="card" style="width: 46rem;">
        <div class="card-header">
          ...
          <h3> Kemaskini Pencapaian </h3>
        </div>
          <!--------------------------RESUME PELAJAR------------------------>
      
      <div class="card-body">
        <form action="pros_kemaskini_pencapaian.php" class="form-control" id="regForm" method="POST" novalidate>

<input type="text" name="id_kebolehan" class="form-control" value="<?php echo $row['id_kebolehan']; ?>" hidden>
        ...
        <div class="col-sm-6">
            <input type="text" name="pencapaian" class="form-control" value="<?php echo $row['pencapaian']; ?>" oninput="this.value = this.value.toUpperCase()" 
             required>
        </div>
      </div>
      ...
      <div class="form-group row">
        <label for="tahap_pencapaian" class="col-sm-3 col-form-label" > Tahap Pencapaian :</label>
        <div class="col-sm-6">
          <input type="text" name="tahap_pencapaian" class="form-control" value="<?php echo $row['tahap_pencapaian']; ?>" >
        </div>
      </div>
